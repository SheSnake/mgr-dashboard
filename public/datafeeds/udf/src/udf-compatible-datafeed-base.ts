...
import { bus, SymbolChangeEvent } from "../../../../src/utils/bus";
...
// it is hack to let's TypeScript make code flow analysis
export interface UdfSearchSymbolsResponseV2 {
  s?: undefined;
  result?: UdfSearchSymbolsResponse;
}
  ...
  implements IExternalDatafeed, IDatafeedQuotesApi, IDatafeedChartApi
{
  ...
  public searchSymbols(
    ...
    if (this._configuration.supports_search) {
      ...
      this._send<UdfSearchSymbolsResponseV2 | UdfErrorResponse>(
        "search",
        params
      )
        .then((response: UdfSearchSymbolsResponseV2 | UdfErrorResponse) => {
          ...
          if (response.result === undefined) {
            console.log("v2mis:", response);
            logMessage(
              `UdfCompatibleDatafeedV2: search no symbols resp=${response}`
            );
            onResult([]);
            return;
          }
  ...
  public resolveSymbol(
    ...
    function onResultReady(symbolInfo: LibrarySymbolInfo): void {
      ...
      console.log("on_resolve_symbol:", symbolInfo);
